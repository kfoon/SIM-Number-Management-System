...
$hlrsql = "UPDATE  unallocate_rep SET switch_tech = '$hlruser', status = 'Done'  WHERE id = '$checkbox' LIMIT 1";
mysql_query($hlrsql);
...
SELECT Customer_Name,new_sim,lost_sim,date_lost,ref_No1,ref_No2,ref_no3,user,date_created,time,status,switch_tech,Address,active,private,mca,crbt FROM unallocate_rep WHERE id = '$checkbox'";
mysql_query($sql);

//$id = $_GET["id"];

$order = "DELETE FROM  unallocate_rep WHERE id = '$checkbox' LIMIT 1"; //  WHERE"; // ID = '$id'";
 mysql_query($order);
 ++$i;
 } 
